...
nmax_pro = multiprocessing.Semaphore(4)
...
def detection(img_path):
        ...
        for fi, p2 in enumerate(p):
            ...
            for ij, dij in enumerate(tmp_disps):
                ...
                if tmp_diff < 3 * alfa_l[li]:  # 根据阈值选d
                    ...
                    wijfl = np.exp(-(tmp_diff ** 2) / (2 * (alfa_l[li] ** 2))) / (tmp_disps.shape[0] + param_fi)
                    weights[li][-1] += wijfl
    ...
    print("total used time is {} min".format((end - start) / 60.0))
...
if __name__ == '__main__':
    ...
    for thd in thds:
        thd.join()
    '''
    p1 = multiprocessing.Process(target=detection, args=("爆片绝缘子.png",))
    p2 = multiprocessing.Process(target=detection, args=("sample.png",))
    p1.start()
    p2.start()
    p1.join()
    p2.join()
